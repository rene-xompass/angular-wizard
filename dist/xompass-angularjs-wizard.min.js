...
function wizardButtonDirective(a) {
  angular.module('xompass-angular-wizard').directive(a, function () {
    return {
...
angular.module('xompass-angular-wizard', ['templates-xompass-angular-wizard']), angular.module('xompass-angular-wizard').directive('wzStep', function () {
  return {
    restrict: 'EA',
    replace: !0,
    transclude: !0,
  ...
  return {
    ...
    controller: ['$scope', '$element', '$log', 'WizardHandler', '$q', '$timeout', function ($scope, $element, $log, WizardHandler, $q, $timeout) {
      function canEnterStep(step) {
        ...
        return 'function' == typeof canEnter.then ? (defer = $q.defer(), canEnter.then(function (a) {
          defer.resolve(a);
      ...
      function canExitStep(step, stepTo) {
        ...
        return 'function' == typeof canExit.then ? (defer = $q.defer(), canExit.then(function (a) {
          defer.resolve(a);
      ...
      $scope.context = {}, $scope.$watch('currentStep', function (a) {
        ...
        if (firstRun) unselectAll(), $scope.selectedStep = a, void 0 !== $scope.currentStep && ($scope.currentStep = a.wzTitle), a.selected = !0, $scope.$emit('wizard:stepChanged', {
          step: a,
          index: stepIdx(a)
        }), firstRun = !1; else {
          var b;
          $scope.currentStepNumber() > 0 ? b = $scope.currentStepNumber() - 1 : 0 === $scope.currentStepNumber() && (b = 0), $q.all([canExitStep($scope.getEnabledSteps()[b], a), canEnterStep(a)]).then(function (b) {
            b[0] && b[1] ? (unselectAll(), $scope.selectedStep = a, angular.isUndefined($scope.currentStep) || ($scope.currentStep = a.wzTitle), a.selected = !0, $scope.$emit('wizard:stepChanged', {
              step: a,
              index: stepIdx(a)
            })) : $scope.$emit('wizard:stepChangeFailed', {step: a, index: ($scope.getEnabledSteps() || []).indexOf(a)});
          });
        }
...
}), wizardButtonDirective('wzNext'), wizardButtonDirective('wzPrevious'), wizardButtonDirective('wzFinish'), wizardButtonDirective('wzCancel'), wizardButtonDirective('wzReset'), angular.module('xompass-angular-wizard').factory('WizardHandler', function () {
  const a = {}, b = {};
  return a.defaultName = 'defaultWizard', a.addWizard = function (a, c) {
    ...
    var d = c || a.defaultName;
    return b[d];
  }, a;
}), angular.module('templates-xompass-angular-wizard', ['step.html', 'wizard.html']), angular.module('step.html', []).run(['$templateCache', function (a) {
